...
*Note*: The truth of $H_0$ for a null hypothesis of the type $\mu = 0$ is somewhat of an abstraction. In practice, the difference between two treatments can be very small, but will almost never be perfectly zero. Nevertheless, one can conceive a null hypothesis that is true up to a certain precision.
...
For two independent samples each following a normal distribution, under the null hypothesis that these two distributions have the same mean, the difference of the estimated means $\bar{x}_A - \bar{x}_B$ divided by its standard error also follows a $t$ distribution:
...
These formulas are presented here for your information, but fortunately, statistical software performs these computations for us. Here is the R code to compare the means of the samples treated by products A and B.
